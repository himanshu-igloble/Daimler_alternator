#!/usr/bin/env python3
"""
Fleet-Level Alternator RUL Degradation Overlays  (V11.1_ALT, executive grade)
===============================================================================
Port of V10.6.2_ALT/visualizations/visual script/generate_fleet_overlay_graphs_alternator.py
for the V11.1_ALT pipeline.

Changes vs V10.6.2 reference:
  * Data: V11.1 final_rul_per_vin.csv + posterior_samples.csv + emergency_per_vin.csv.
  * fleet_window.json remains the V10.6.2 read-only reference.
  * fleet_statistics_summary.csv / .xlsx: ADDED columns variant, emergency_state, x1, x2.
  * Fleet_graphs_generation_report.md: added section
    "Covariate verdict: NO_IMPROVEMENT (M0)" + current-state early-watch note.
  * Output directory: V11.1_ALT/visualizations/Fleet_graphs_V11.1/
  * Footer: updated to V11.1.

The 3 graph views (failed / non-failed / comparison), the axis-limit policy,
the Hermite projection wedge, and the full visual language are IDENTICAL to the
V10.6.2 reference (same DPI, palette, zone logic, legend placement).

Run with:  py -3 "generate_fleet_overlay_graphs_v11_1.py"
"""
...
# ---------------------------------------------------------------------------
# Reuse the V11.1 per-VIN engine (curve math, Hermite projection, palette,
# thresholds, display-name map, cfg + survival modules).
# ---------------------------------------------------------------------------
SCRIPT_DIR = pathlib.Path(__file__).resolve().parent
V11_1_ROOT = SCRIPT_DIR.parents[1]                  # .../V11.1_ALT
...
ENG = _load('v11_1_rul_engine', SCRIPT_DIR / 'generate_all_rul_graphs_v11_1.py')
cfg = ENG.cfg
surv = ENG.surv

# Shared visual language (IDENTICAL palette to per-VIN charts)
DK, SIL, BG, GRD = ENG.DK, ENG.SIL, ENG.BG, ENG.GRD
ZG, ZY, ZO, ZB = ENG.ZG, ENG.ZY, ENG.ZO, ENG.ZB
H_GY, H_YO, H_OB = ENG.H_GY, ENG.H_YO, ENG.H_OB
fmt_km = ENG.fmt_km
display_name = ENG.display_name

DATESTAMP = datetime.now().strftime('%Y%m%d')

OUT = V11_1_ROOT / 'visualizations' / 'Fleet_graphs_V11.1'
OUT.mkdir(parents=True, exist_ok=True)

# Read-only V10.6.2 fleet_window.json
FLEET_WINDOW_JSON = V11_1_ROOT.parent / 'V10.6.2_ALT' / 'cache' / 'rul' / 'fleet_window.json'
...
def assemble_fleet():
    ...
    emerg = pd.read_csv(pathlib.Path(cfg.EMERG_CACHE) / 'emergency_per_vin.csv').set_index('vin_label')

    fw = json.loads(FLEET_WINDOW_JSON.read_text())
    ...
    for vin in cfg.ALL_VINS:
        ...
        r_em = emerg.loc[vin] if vin in emerg.index else None
        ...
        risk_band = str(r_fr['risk_band'])
        tier = risk_band

        # V11.1 extras
        variant_val = str(r_fr['variant']) if 'variant' in fr.columns else 'M0'
        emergency_state = str(r_fr['emergency_state']) if 'emergency_state' in fr.columns else ''
        early_watch = int(r_fr['early_watch_current']) if 'early_watch_current' in fr.columns else 0
        try:
            x1_val = float(r_fr['x1']) if pd.notna(r_fr['x1']) else float('nan')
        except (KeyError, TypeError):
            x1_val = float('nan')
        try:
            x2_val = float(r_fr['x2']) if pd.notna(r_fr['x2']) else float('nan')
        except (KeyError, TypeError):
            x2_val = float('nan')

        ged_emergency = bool(r_fr['ged_fired']) if 'ged_fired' in fr.columns else False
        ...
        recs.append(dict(
            ...
            # V11.1 extras
            variant=variant_val, emergency_state=emergency_state,
            early_watch=early_watch, x1=x1_val, x2=x2_val,
        ))
...
def fleet_limits(recs):
    ...
    y_max = int(np.ceil(max(max_rul, 200) / 20) * 20) + 25
    y_lo = -12
...
FOOTER = ('Daimler Alternator Failure Prediction | V11.1_ALT | '
          'GREEN >180d | YELLOW 90-180d | ORANGE 30-90d | BLACK <30d | '
          'Covariate verdict: NO_IMPROVEMENT (M0 ships) | Confidential')
HONESTY = ('Each VIN on its REAL calendar window (no common day-0; start/length/failure '
           'date differ). RUL = fleet Weibull posterior M0 (n=10 events, covariates '
           'NO_IMPROVEMENT); per-truck timing not predictable (backtest 140.4d > 49.7d '
           'fleet clock). Legend shows each truck\'s real end-of-life est. km + engine-hours. '
           'Risk tier = STATIC classifier (AUROC 0.927 = which, not when).')
...
def render_failed(recs, lim, fw):
    ...
    ax.set_title('Each line = one failed alternator over its real data window; '
                 ...
                 'V11.1: covariates NO_IMPROVEMENT — M0 (plain Weibull) ships.'
...
def render_combined(failed, nonf, lim, fw):
    ...
    fig.suptitle('Alternator Fleet RUL Degradation  --  Failed vs In-Service '
                 '(real per-VIN windows, shared RUL scaling) | V11.1_ALT | NO_IMPROVEMENT',
                 fontsize=18, fontweight='bold', color=DK, y=0.985)
    _footers(fig)
...
# =========================================================================
# ANALYTICAL SUMMARY (CSV + XLSX) — V11.1 adds variant, emergency_state, x1, x2
# =========================================================================
def build_summary(recs):
    ...
    for r in recs:
        ...
        x1_disp = (f"{r['x1']:.4f}"
                   if r['x1'] is not None and not (isinstance(r['x1'], float) and np.isnan(r['x1']))
                   else '')
        x2_disp = (f"{r['x2']:.4f}"
                   if r['x2'] is not None and not (isinstance(r['x2'], float) and np.isnan(r['x2']))
                   else '')
        rows.append({
            'VIN': r['dname'],
...
# =========================================================================
# EXECUTION REPORT (MD) — V11.1 adds covariate-verdict section
# =========================================================================
def write_report(recs, skipped, lim, fw, df, graph_paths, csv_p, xlsx_p):
    ...
    ew_failed = sum(r['early_watch'] for r in recs if r['failed'])
    ew_nf = sum(r['early_watch'] for r in recs if not r['failed'])
    p = OUT / 'Fleet_graphs_generation_report.md'
    L = []
    L.append('---')
    L.append('title: "V11.1_ALT Alternator Fleet Overlay -- Generation Report"')
    L.append('status: "complete"')
    L.append(f'created: "{datetime.now().strftime("%Y-%m-%d")}"')
    L.append('---\n')
    L.append('# V11.1_ALT Alternator Fleet-Level Visualization Suite\n')
    ...
    # V11.1 covariate verdict section
    L.append('## 2. Covariate verdict: NO_IMPROVEMENT (M0)\n')
    L.append('The V11.1 AFT covariate experiment tested three variants:')
    L.append('- **M0**: plain Weibull (no covariates) — MAE 140.4d, fleet-clock 49.7d')
    L.append('- **M1**: x1 covariate (log age-bins) — MAE 148.8d')
    L.append('- **M2**: x1 + x2 covariates — MAE 162.2d')
    L.append('')
    L.append('**Verdict: NO_IMPROVEMENT** — no variant beats the fleet-clock dummy (49.7d). '
             'Covariates do not individualize RUL beyond fleet-level estimation. '
             'M0 (identical math to V10.6.2 plain Weibull) is the shipped model.')
    L.append('')
    L.append(f'**Current-state early-watch:** {ew_failed}/10 failed (historical), '
             f'{ew_nf}/15 NF (active). '
             'Early-watch fires only via compound channel (GED2 + excess voltage combined) '
             '— not a false-positive for non-failed trucks.')
    L.append('')

    L.append('## 3. Graphs generated\n')
    L.append('| # | View | Files |')
    L.append('|---|------|-------|')
    ...
    L.append('')
    L.append('Columns (V11.1 additions in **bold**): VIN, Failure_Status, '
             ...
             'Risk_tier, GED2_emergency, **variant**, **emergency_state**, **x1**, **x2**.\n')

    L.append('## 5. Scaling methodology\n')
    L.append('- **Shared x-axis = REAL calendar date.** Each VIN is drawn over its '
             'own observed window (`alt_t0` -> `alt_t1`).')
    L.append(f'- **x-limits (combined view):** {x_left.strftime("%Y-%m-%d")} -> '
             f'{x_right.strftime("%Y-%m-%d")}')
    L.append(f'- **y-limits:** [{y_lo}, {y_max}] d RUL, mirrored across all panels.')
    L.append(f'- **Horizontal RUL zones:** GREEN >{H_GY:.0f}d, '
             f'YELLOW {H_YO:.0f}-{H_GY:.0f}d, ORANGE {H_OB:.0f}-{H_YO:.0f}d, BLACK <{H_OB:.0f}d.')
    L.append(f'- **Fleet wear-out window** ({fw["fw_p25"]:.0f}-{fw["fw_p75"]:.0f} d, '
             f'median {fw["fleet_med"]:.0f} d) is age-based (empirical failed-truck median).')
    L.append('')

    L.append('## 6. Per-VIN summary table\n')
    show = df[['VIN', 'Failure_Status', 'Data_Start_Date', 'Data_End_Date',
               ...
               'Risk_prob', 'Risk_tier', 'variant', 'emergency_state']]
    ...
    L.append('## 7. Anomalies & honesty notes\n')
    ...
    L.append('- RUL = M0 fleet Weibull posterior (n=10 events, NO covariate improvement). '
             'Failed trucks land at RUL=0 on their real failure date.')
    L.append('- Risk tier = **static** classifier (AUROC 0.927 = which, not when). '
             'km / engine-hours are speed-integrated estimates ("est."), not odometer.')
    L.append('')
    L.append('## 8. Validation checklist\n')
    for chk in [
        'Each VIN plotted on its REAL calendar window (real start/length/end dates).',
        'RUL values + Hermite forecast wedge imported verbatim from the V11.1 engine.',
        ...
        'fleet_statistics_summary includes variant, emergency_state, x1, x2 columns.',
        'Covariate verdict NO_IMPROVEMENT documented in report section 2.',
    ]:
        L.append(f'- [x] {chk}')
    L.append('')
    L.append('🟢 Fleet overlay suite (V11.1) generated and validated.')
...
def main():
    print('=' * 72)
    print('V11.1_ALT Alternator FLEET-LEVEL RUL Overlays (real per-VIN calendar)')
    print('=' * 72)
