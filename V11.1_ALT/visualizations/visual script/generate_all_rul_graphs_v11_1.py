#!/usr/bin/env python3
"""
Generate Alternator RUL Degradation Graphs for ALL VINs  (V11.1 — executive grade)
====================================================================================
Port of V10.6.2_ALT/visualizations/visual script/generate_all_rul_graphs_alternator.py
for the V11.1_ALT pipeline.  The visual design (22x12 figure, palette, typography,
phase bands, threshold markers, projection wedge, legend, footer) is IDENTICAL to the
V10.6.2 chart.  Changes:

  * Data sources: V11.1 final_rul_per_vin.csv + V11.1 posterior_samples.csv
                  + V10.6.2's fleet_window.json (read-only) + lifecycle + weekly parquets.
  * Legend block: ADDED lines "variant: M0 (NO_IMPROVEMENT)" and "early-watch: yes/no"
                  + x1/x2 values from final_rul_per_vin.csv.
  * Footer: updated to V11.1.
  * GED overlay only for VIN1_F_ALT and VIN10_F_ALT (same verified precursors as V10.6.2).

Run with:  py -3 "generate_all_rul_graphs_v11_1.py"
"""
...
def create_graph(weeks, rul_med, rul_p10, rul_p90, vin_id,
    ...
    # V11.1 additions
    entries += [
        Line2D([0], [0], color='none', label=f'variant: {variant} (NO_IMPROVEMENT)'),
        Line2D([0], [0], color='none', label=f'early-watch: {ew_label}'),
        Line2D([0], [0], color='none', label=f'x1={x1_str}  x2={x2_str}'),
    ]
    ...
    fig.text(0.02, 0.012,
             'Daimler Alternator Failure Prediction | V11.1_ALT | '
    ...
    fig.text(0.02, 0.035,
             'RUL = fleet Weibull posterior M0 (n=10 events, covariates NO_IMPROVEMENT); '
             'per-truck timing not predictable (backtest 140.4d > 49.7d fleet clock). '
             'Risk tier = STATIC classifier (AUROC 0.927 = which, not when). km estimated.',
