#!/usr/bin/env python3
"""
Generate Alternator Remaining-Useful-Life Graphs for ALL VINs  (V11.1 — BUSINESS edition)
=========================================================================================
Business-oriented variant of generate_all_rul_graphs_v11_1.py.  The visual design
(22x12 figure, palette, typography, phase bands, threshold markers, projection wedge,
legend, footer, curves and data) is IDENTICAL — only the LABELLING is made
business-friendly:

  * No model details: removes "Weibull", "posterior", "variant/M0", "NO_IMPROVEMENT",
    "early-watch", "x1/x2", backtest numbers, AUROC, and the raw risk probability.
  * Plain-language labels: "Predicted remaining life", "Likely range (80% confidence)",
    "Risk level: <tier>", "Failure recorded", "Typical replacement window".
  * Fixed release date: graphs are stamped Generated 2026-06-18 and filenames carry
    the 20260618 datestamp.
  * Output folder: visualizations/rul_curves (business graphs replace the
    earlier technical-labelled set).

Run with:  py -3 "generate_business_rul_graphs_v11_1.py"
"""
...
# -- Datestamp for output filenames (fixed: business release 2026-06-18) -----
DATESTAMP = '20260618'
GENERATED_DATE = '2026-06-18'

# -- Paths --------------------------------------------------------------------
#   __file__ = .../V11.1_ALT/visualizations/visual script/<this>.py
SCRIPT_DIR = pathlib.Path(__file__).resolve().parent
V11_1_ROOT = SCRIPT_DIR.parents[1]                  # .../V11.1_ALT
SRC = V11_1_ROOT / 'src'
# Read-only V10.6.2 fleet_window.json
V10_6_2_ROOT = V11_1_ROOT.parent / 'V10.6.2_ALT'
FLEET_WINDOW_JSON = V10_6_2_ROOT / 'cache' / 'rul' / 'fleet_window.json'

OUT = V11_1_ROOT / 'visualizations' / 'rul_curves'
OUT.mkdir(parents=True, exist_ok=True)


def _load(mod, fn, base=SRC):
    """Import a source module by path."""
    spec = importlib.util.spec_from_file_location(mod, str(base / fn))
...
surv = _load('V11_1_ALT_survival', 'V11_1_ALT_survival.py')
...
# -- Color palette (IDENTICAL to the V10.6.2 clutch script) ------------------
...
# =============================================================================
# CORE GRAPH — executive grade (V11.1 alternator)
# =============================================================================
def create_graph(weeks, rul_med, rul_p10, rul_p90, vin_id,
                 *, failed, age_now, t0, last_dt, fc_dt,
                 ...
                 variant, early_watch, x1, x2):
    """Executive-grade alternator RUL degradation visualisation (V11.1)."""
    ...
    # =========================================================================
    # 2. HORIZONTAL RUL-HORIZON ZONE BANDS
    # =========================================================================
    hz_a = 0.05
    ...
    # =========================================================================
    # 3. RUL CURVE (conditional posterior median + band)
    # =========================================================================
    ...
    # =========================================================================
    # 4. RUL-HORIZON CROSSINGS + arrows
    # =========================================================================
    if len(proj_d) > 0:
    ...
    if failed:
        ...
        ax.scatter([last_dt], [0], s=300, color='#8B0000', edgecolors=DK, lw=2,
                   zorder=10, marker='X')
        lead_txt = (f'  ~{lead_days:.0f} d early warning' if (has_ged and lead_days)
                    else '  no early warning')
        km_ehrs_txt = f'\n{fmt_km(failure_km)} km  |  {failure_ehrs:,.0f} engine-hrs'
        ax.annotate(f'FAILURE RECORDED\nremaining life = 0{lead_txt}{km_ehrs_txt}',
    ...
    ax.set_ylabel('Predicted Remaining Life (Days)', fontsize=13, fontweight='bold',
    ...
    # =========================================================================
    # 9. LEGEND  (V11.1: adds variant, early-watch, x1, x2 lines)
    # =========================================================================
    ew_label = 'yes' if early_watch else 'no'
    x1_str = f'{x1:.4f}' if x1 is not None and not (isinstance(x1, float) and np.isnan(x1)) else 'n/a'
    x2_str = f'{x2:.4f}' if x2 is not None and not (isinstance(x2, float) and np.isnan(x2)) else 'n/a'

    entries = [
        Line2D([0], [0], color=PRD, lw=2.5, marker='D', ms=4.5,
               markeredgecolor='w', markeredgewidth=0.5,
               label='Predicted remaining life (most likely)'),
        Line2D([0], [0], color=PRD, lw=6, alpha=0.18,
               label='Likely range (80% confidence)'),
    ]
    if len(proj_d) > 0:
    ...
    fig.suptitle(f'Alternator Remaining Useful Life  --  {vin_id}', fontsize=18,
                 fontweight='bold', color=DK, y=0.97)
    if failed:
        sub = (f'Age {age_now:.0f}d  |  {fmt_km(failure_km)} km  |  '
               f'{failure_ehrs:,.0f} eng-hrs  |  Risk level: {tier}  |  '
               f'Failure recorded (remaining life = 0)  |  '
               f'Typical replacement window {fw_p25:.0f}-{fw_p75:.0f}d '
               f'(median {fleet_med:.0f})  |  Status: {status}')
    else:
        sub = (f'Age {age_now:.0f}d  |  Risk level: {tier}  |  '
               f'Predicted remaining life {median_rul:.0f}d (range {p10_now:.0f} - '
               f'{p90_now:.0f}d)  |  Est. at failure: {fmt_km(failure_km)} km, '
               f'{failure_ehrs:,.0f} eng-hrs  |  Typical replacement window {fw_p25:.0f}-{fw_p75:.0f}d '
               f'(median {fleet_med:.0f})  |  Status: {status}')
    ax.set_title(sub, fontsize=10, color='#5D6D7E', style='italic', pad=14)

    fig.text(0.02, 0.012,
             'Daimler Alternator Failure Prediction  |  '
    ...
    fig.text(0.02, 0.035,
             'Remaining life and failure date are fleet-based estimates for maintenance '
             'planning; actual timing varies by vehicle. Distance (km) is estimated. '
             'Colour bands indicate recommended action urgency.',
...
# =============================================================================
# DISPLAY-NAME MAPPING (unique 1-25 numbering)
# =============================================================================
_DISPLAY_NAME = {}
...
def main():
    print('=' * 70)
    print('V11.1 Alternator RUL Degradation Graphs -- Executive Grade (All 25 VINs)')
    print('=' * 70)
    ...
    # V11.1 posterior samples (M0 = no-covariate, same as V10.6.2 mathematically)
    ...
    for vin in cfg.ALL_VINS:
        ...
        tier = risk_band  # risk_band is the canonical tier in V11.1

        # V11.1-specific fields
        variant = str(r_fr['variant']) if 'variant' in fr.columns else 'M0'
        ...
        # GED emergency: only VIN1_F_ALT and VIN10_F_ALT have verified precursors
        ...
        try:
            ...
            if r_em is not None:
                _ged_lead = r_em.get('ged_lead_days', None)
                if _ged_lead is not None and pd.notna(_ged_lead):
                    lead_days = float(_ged_lead)
        ...
        # GED2 storm weeks — verified precursors only
        ged_burst_dates = []
        ...
        fig = create_graph(
            ...
            variant=variant, early_watch=bool(early_watch),
            x1=x1_val, x2=x2_val)
